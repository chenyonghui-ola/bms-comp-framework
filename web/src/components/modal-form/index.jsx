...
import { filterObj } from "../../library/utils";
import "./index.less";
...
export default ({
    ...
    const handleOk = async () => {
        ...
        if (flag == "csv") {
            onCsvOk(filterObj({ ...final_result, ...mediaRef.current }));
        } else {
            onOk(filterObj(final_result));
        }
    ...
    return (
        <Modal
            title={title ? $I18N.t(title) : $I18N.t("新增")}
            visible={visible}
            width={width}
            onOk={handleOk}
            onCancel={onCancel}
        ...
        >
            ...
            <Form {...layout} form={form} preserve={false} onValuesChange={onFieldsChange}>
                {options.map((item, index) => {
                    ...
                    const lang_label = $I18N.t(item.label);
                    const lang_please = $I18N.t("请选择");
                    const lang_input = $I18N.t("请输入");
                    const lang_error_rule = $I18N.t("格式不正确");
                    if (item.type === "switch") {
                        return (
                            <Form.Item
                                key={index}
                                label={lang_label}
                                name={item.name}
                                rules={[{ required: required, message: lang_please }]}
                            >
                                <Switch />
                            </Form.Item>
                        );
                    ...
                    if (item.type === "input") {
                        let rules = [{ required: required, message: lang_input }];
                        if (item.pattern) {
                            rules = [...rules, { pattern: item.pattern, message: lang_error_rule }];
                        }
                        return (
                            <Form.Item key={index} label={lang_label} name={item.name} hidden={item.hidden} rules={rules}>
                                <Input
                                    prefix={item.prefix}
                    ...
                    if (item.type === "rangeinput") {
                        ...
                        return (
                            <Form.Item key={index} label={lang_label} hidden={item.hidden} required={required}>
                    ...
                    if (item.type === "textarea") {
                        return (
                            <Form.Item
                                ...
                                name={item.name}
                                hidden={item.hidden}
                                rules={[{ required: required, message: lang_input }]}
                            >
                                <TextArea placeholder={item.placeholder} autoSize={{ minRows: 4 }} disabled={item.disabled} />
                            </Form.Item>
                        );
                    }
                    ...
                    if (item.type === "upload") {
                        return (
                            <Form.Item label={lang_label} required={required} key={index} hidden={item.hidden}>
                                <div
                                    style={{
                                        display: "flex",
                                        flexWrap: "nowrap",
                                ...
                                >
                                    ...
                                    <If data={item.includeClear}>
                                        <Button onClick={() => handleUploadClear(item.name)}>{$I18N.t("清除")}</Button>
                                    </If>
                                </div>
                            ...
                            <Form.Item label={lang_label} required={required} key={index} hidden={item.hidden}>
                                ...
                                >
                                    ...
                                    <Upload fileList={[]} customRequest={({ file }) => handleBeforeUploadCSV(file, item.name)}>
                                        <Button type="primary"> {$I18N.t("上传")}</Button>
                                    </Upload>
                                </div>
                            </Form.Item>
                        );
                    ...
                    if (item.type == "multipleUpload") {
                        return (
                            ...
                            >
                                ...
                                >
                                    {fileList.length >= 8 || item.disabled ? null : uploadButton}
                                </Upload>
                            </Form.Item>
                        );
                    }
                    ...
                    if (item.type === "datepicker") {
                        return (
                            ...
                            >
                                <DatePicker
                                    picker={item.flag || "date"}
                                    showTime={!item.notShowTime}
                                    disabled={item.disabled}
                    ...
                    if (item.type === "text") {
                        return (
                            <Form.Item key={index} label={lang_label} hidden={item.hidden} name={item.name} rules={[{ required: false }]}>
                    ...
                    if (item.type === "timepicker") {
                        return (
                            <Form.Item
                                ...
                                label={lang_label}
                                name={item.name}
                                rules={[{ required, message: lang_please }]}
                            >
                    ...
                    if (item.type == "onlyshow") {
                        return (
                            <Form.Item
                                ...
                                rules={[{ required: false, message: lang_please }]}
                            >
                                {item.content}
                            </Form.Item>
                        );
