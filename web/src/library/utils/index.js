...
export  const formaturl=(url,obj)=>{
    // todo
}
...
export const formatStr = str => {
    if (!str) return 0;
    let num = str;
    if (localStorage.getItem("hiddenDot") != "0") {
        num = `${parseInt(str)}`;
    }
...
/**
 *导出excel, url必须以/开头,如 /api/xxxx
 * @param {*} functions
 * @returns
 */
export const exportExcel = url => {
    let exportPrefix = cfg.ajaxPrefix || "http:///";
    const a = document.createElement("a");
    a.setAttribute("href", `${exportPrefix}${url}`);
