...
import { isLogin } from "src/commons/loginFn";
...
export default class KeepAuthRoute extends React.Component {
    async componentDidMount() {
        ...
        //     const { data } = await ajax.get("/api/user/permission");
        //     user.setPermissions(data.map(item => item.path));
        // }
    }
    ...
    render() {
        ...
        return (
            <CacheRoute
                ...
                cacheKey={rest.path}
                render={props => {
                    const configKeepAlive = keepAliveRoutes.find(
                        item => item.path === rest.path
                    )?.keepAlive;
                    const keepAlive =
                        configKeepAlive === void 0 ? keepAliveSystem : configKeepAlive;
                    const { history } = props;
                    const {
                        action: { system }
                    } = this.props;
                    ...
                    if (tabsShow || keepAlive || keepAliveRoutes.length) {
                        ...
                        const currentPath = window.decodeURIComponent(
                            `${pathname}${search}`
                        );
                        ...
                        if (nextActiveTab) {
                            ...
                            setTimeout(() => {
                                ...
                                if (path.startsWith(iframePagePrefix)) {
                                    path = `${iframePagePrefix}${window.encodeURIComponent(
                                        path.replace(iframePagePrefix, "")
                                    )}`;
                                }
                                history.push(path);
                            });
                        ...
                        if (currentTab && !currentTab.active) {
                            ...
                            // 记录上一个页面的滚动条位置
                            if (prevActiveTab) {
                                prevActiveTab.scrollTop = document.body.scrollTop =
