...
trait ValidationTrait
{
    ...
    public function validationFieldLength($field, $fieldName = null, $length = 0)
    {
        ...
        $strlen = Helper::strlen($this->context->{$field});
    ...
    public function validationFieldCount($field, $fieldName = null, $length = 0)
    {
        if ($length <= 0) {
            return;
        }

        $count = count($this->context->{$field});

        if ($count <= $length) {
            return;
        }

        [$code, $msg] = CommonException::COMMON_FIELD_COUNT_MAX;
        throw new CommonException(sprintf($msg, $fieldName, $length), $code);
    }
    ...
    public function validationFieldEnum($field, $fieldName = null, $enum = [])
    {
        ...
        if ((is_numeric($this->context->{$field}) || isset($this->context->{$field})) && (isset($enum[$this->context->{$field}]) || ((isset($enum[0]) && in_array($this->context->{$field}, $enum))))) {
            return;
        }
