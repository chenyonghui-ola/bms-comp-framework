...
$params = array();
if (isset($argv[1])) $arguments['task'] = $argv[1];
if (count($argv) >= 2) {
...
try {
    LoggerProxy::instance()->warning("[" . implode(',', $params) . "]");
    ...
    $formatter = new LineFormatter("[%date%][{$uuid}][Cli] - %message%");
    ...
    $msg = $msg . "\n" . $e->getTraceAsString();
    $logger->error("[{$arguments['task']}/{$arguments['action']}][" . implode(',', $params) . "]" . $msg);
    exit(255);
}