...
use Imee\Service\Event\DbEventListener;
use Phalcon\Di;
...
$di->set('logger', function () use ($uuid) {
    $logger = new Imee\Libs\Fixed\File(ROOT . DS . 'cache' . DS . 'log' . DS . 'admin_debug.log');
...
$di->set('dblogger', function () use ($uuid) {
    $logger = new Imee\Libs\Fixed\File(ROOT . DS . 'cache' . DS . 'log' . DS . 'admin_db.log');
    $formatter = new LineFormatter("[%type%][%date%][{$uuid}] - %message%");
    $logger->setFormatter($formatter);
    $logger->setLogLevel(defined('DEBUG') && DEBUG ? Logger::INFO : Logger::WARNING);
    $logger->begin();
